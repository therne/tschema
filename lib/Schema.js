...
class Schema {
    constructor(schema, options = {}) {
        ...
        this.validationFunction = new Function('x', code).bind(this);
    }
    ...
    compileCode(schema, field = 'x', fieldName = 'value') {
        const raiseError = type => `return this.options.errorProducer('${fieldName}', '${type}')`;
        ...
        if (schema === Number || schema === 'number') {
            ...
            return `if (!(${field} instanceof Date) && `
                + `isNaN(this.options.dateParser(${field}).getTime())) ${raiseError('Date')};`;
            ...
            return `if (!(${field} instanceof Array)) ${raiseError('Array')};`
                + `${field}.forEach((item) => { ${innerSchemaValidationCode} });`;
